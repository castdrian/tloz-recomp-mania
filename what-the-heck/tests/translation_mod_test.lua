...
local applied = {}
local expected = {
  red = { text = 2582, pokemon = 151, moves = 165, items = 152, trainers = 47 },
  blue = { text = 2582, pokemon = 151, moves = 165, items = 152, trainers = 47 },
  yellow = { text = 2706, pokemon = 151, moves = 165, items = 152, trainers = 47 },
  gold = { text = 3042, pokemon = 251, moves = 251, items = 247, trainers = 562 },
}
...
for _, version in ipairs({ "red", "blue", "yellow", "gold" }) do
  ...
  for _, value in pairs(applied.text) do
    ...
    assert(not value:find("I-", 1, true), version .. " text contains a relay I- artifact")
  end
  ...
  for _, name in ipairs({ "pokemon", "moves", "items", "trainers" }) do
    ...
    for _, value in pairs(applied[name]) do
      ...
      assert(not value:find("I-", 1, true), version .. " " .. name .. " contains an unexpected I- artifact")
    end
  end
end
